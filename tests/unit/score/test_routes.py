...
def test_stream_score(client: TestClient, valid_stream_input, valid_stream_output):
    ...
    valid_stream_output[0]["input"] = valid_stream_input["patient1"][0]
    assert results == valid_stream_output
...
def test_invalid_input_data_type(client:TestClient, invalid_input_error_message):
    params = {"modelVersion": "v1"}
    input = {"patient1": ["0.03807591"]}
    response = client.post(STREAM_PATH, params=params, json=input)
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
