...
from app.errors import ModelTrainingFailed
...
def evaluate(model, X, y, y_predicted):
    ...
    if r2 >= 0.4:
        ...
        raise ModelTrainingFailed(f"Failed training: {r2=}")


def save_model(model, model_version):
    filename = f"db_score:{model_version}.sav"
    filepath = f"app/model/{filename}"
